...
export default function InfoTooltip({ text, width = 'w-64', align = 'center' }: Props) {
  ...
  return (
    <span ref={ref} className="relative inline-flex items-center ml-1">
      ...
      {open && (
        <span className={`pointer-events-none absolute bottom-full mb-2 ${width} z-20 ${align === 'right' ? 'right-0 sm:right-auto sm:left-1/2 sm:-translate-x-1/2' : 'left-0 sm:left-1/2 sm:-translate-x-1/2'}`}>
